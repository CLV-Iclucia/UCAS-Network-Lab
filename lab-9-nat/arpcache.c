...
void arpcache_append_packet(iface_info_t *iface, u32 ip4, char *packet, int len)
{
	...
	log(DEBUG, "create new pending arp request, iface %s, ip "IP_FMT, iface->name, HOST_IP_FMT_STR(ip4));
		...
		list_for_each_entry_safe(req_entry, req_q, &(arpcache.req_list), list) {
			if (time(NULL) - req_entry->sent >= 1) {
				if (req_entry->retries >= ARP_REQUEST_MAX_RETRIES) {
					...
					log(DEBUG, "retransmit arp request to " IP_FMT, HOST_IP_FMT_STR(req_entry->ip4));
