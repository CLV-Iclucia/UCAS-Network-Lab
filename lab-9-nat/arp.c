...
void arp_send_request(iface_info_t* iface, u32 dst_ip) {
  log(DEBUG, "send arp request to "IP_FMT, HOST_IP_FMT_STR(dst_ip));
...
void handle_arp_packet(iface_info_t* iface, char* packet, int len) {
  struct ether_arp* arp_hdr = (struct ether_arp *)(packet + ETHER_HDR_SIZE);
  log(DEBUG, "received arp packet from "IP_FMT, NET_IP_FMT_STR(arp_hdr->arp_spa));
...
void iface_send_packet_by_arp(iface_info_t* iface, u32 dst_ip, char* packet, int len) {
  ...
  if (found) {
    log(DEBUG, "found mac for "IP_FMT" in arpcache, send packet", HOST_IP_FMT_STR(dst_ip));
    memcpy(eh->ether_shost, iface->mac, ETH_ALEN);
    memcpy(eh->ether_dhost, dst_mac, ETH_ALEN);
    iface_send_packet(iface, packet, len);
  }
  else {
    log(DEBUG, "not found mac for "IP_FMT" in arpcache, pending packet", HOST_IP_FMT_STR(dst_ip));
